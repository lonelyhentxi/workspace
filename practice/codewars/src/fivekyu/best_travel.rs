...
pub fn choose_best_sum(max_miles: i32, towns: i32, ls: &Vec<i32>) -> i32 {
    if (ls.len() as i32) < towns || towns<=0 || max_miles<=0 {
        return -1;
    }
    let sums = choose_best_sum_recursively(0 as usize,towns as usize,ls);
