...
export class StartupService {
  ...
  private viaServer(resolve: any, reject: any) {
    ...
    zip(
      ...
      this.httpClient.get(this.apiService.apiJoin('users/2a2eec6f-58f5-44e1-95e8-887955c17533')),
      this.httpClient.get(this.apiService.apiJoin('applications')),
    ).pipe(
      catchError(([postCategories, userInfo, applications]) => {
        resolve(null);
        return [postCategories, userInfo, applications];
      }),
    ).subscribe(([postCategoriesRes, userInfoRes, applicationsRes]) => {
        const postCategories: { Name: string, Description: string, Id: number }[] = postCategoriesRes.body;
        const userInfo: { Name: string, Avatar: string } = userInfoRes.body;
        this.apiService.apiCache.applications.splice(0,0,...applicationsRes.body);
        ...
        this.settingService.setApp(app);
        // 用户信息：包括姓名、头像
        this.settingService.setUser({ name: userInfo.Name, avatar: userInfo.Avatar });
        // ACL：设置权限为全量
        this.aclService.setFull(true);
        // 初始化菜单
