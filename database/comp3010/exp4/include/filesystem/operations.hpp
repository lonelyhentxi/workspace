#ifndef TINY_DB_ENGINE_FILESYSTEM3_OPERATIONS_HPP
#define TINY_DB_ENGINE_FILESYSTEM3_OPERATIONS_HPP

# if defined( TINY_DB_ENGINE_NO_STD_WSTRING )
...
#ifdef TINY_DB_ENGINE_WINDOWS_API
#  include <fstream>
#endif
...
namespace boost
{
  namespace filesystem
  {
    ...
    class TINY_DB_ENGINE_SYMBOL_VISIBLE filesystem_error : public system::system_error
    {
      // see http://www.boost.org/more/error_handling.html for design rationale

      // all functions are inline to avoid issues with crossing dll boundaries

      // functions previously throw() are now TINY_DB_ENGINE_NOEXCEPT_OR_NOTHROW
      // functions previously without throw() are now TINY_DB_ENGINE_NOEXCEPT

    public:
      // compiler generates copy constructor and copy assignment

      filesystem_error(
        const std::string & what_arg, system::error_code ec) TINY_DB_ENGINE_NOEXCEPT
      ...
      filesystem_error(
        ...
        system::error_code ec) TINY_DB_ENGINE_NOEXCEPT
        : system::system_error(ec, what_arg)
      {
        try
        {
      ...
      filesystem_error(
        const std::string & what_arg, const path& path1_arg,
        const path& path2_arg, system::error_code ec) TINY_DB_ENGINE_NOEXCEPT
        : system::system_error(ec, what_arg)
      {
        try
        {
      ...
      ~filesystem_error() TINY_DB_ENGINE_NOEXCEPT_OR_NOTHROW{}

        const path& path1() const TINY_DB_ENGINE_NOEXCEPT
      {
        static const path empty_path;
        return m_imp_ptr.get() ? m_imp_ptr->m_path1 : empty_path;
      }
        const path& path2() const  TINY_DB_ENGINE_NOEXCEPT
      {
        static const path empty_path;
        return m_imp_ptr.get() ? m_imp_ptr->m_path2 : empty_path;
      }

        const char* what() const TINY_DB_ENGINE_NOEXCEPT_OR_NOTHROW
      {
        if (!m_imp_ptr.get())
...
//--------------------------------------------------------------------------------------//
  ...
  enum perms
  {
    ...
    // TINY_DB_ENGINE_BITMASK op~ casts to int32_least_t, producing invalid enum values
    _detail_extend_perms_32_1 = 0x7fffffff,
    _detail_extend_perms_32_2 = -0x7fffffff-1
  };

  TINY_DB_ENGINE_BITMASK(perms)

//--------------------------------------------------------------------------------------//
//                                    file_status                                       //
//--------------------------------------------------------------------------------------//

  class TINY_DB_ENGINE_FILESYSTEM_DECL file_status
  {
  public:
             file_status() TINY_DB_ENGINE_NOEXCEPT
               : m_value(status_error), m_perms(perms_not_known) {}
    explicit file_status(file_type v) TINY_DB_ENGINE_NOEXCEPT
               : m_value(v), m_perms(perms_not_known)  {}
             file_status(file_type v, perms prms) TINY_DB_ENGINE_NOEXCEPT
  ...
  //  functions. GCC is not even consistent for the same release on different platforms.
    ...
    file_status(const file_status& rhs) TINY_DB_ENGINE_NOEXCEPT
      : m_value(rhs.m_value), m_perms(rhs.m_perms) {}
    file_status& operator=(const file_status& rhs) TINY_DB_ENGINE_NOEXCEPT
    {
      m_value = rhs.m_value;
      m_perms = rhs.m_perms;
      return *this;
...
# if !defined(TINY_DB_ENGINE_NO_CXX11_RVALUE_REFERENCES)
    file_status(file_status&& rhs) TINY_DB_ENGINE_NOEXCEPT
    {
      m_value = std::move(rhs.m_value);
      m_perms = std::move(rhs.m_perms);
    }
    file_status& operator=(file_status&& rhs) TINY_DB_ENGINE_NOEXCEPT
    { 
      m_value = std::move(rhs.m_value);
      m_perms = std::move(rhs.m_perms);
      return *this;
...
# endif
    ...
    // observers
    file_type  type() const TINY_DB_ENGINE_NOEXCEPT            { return m_value; }
    perms      permissions() const TINY_DB_ENGINE_NOEXCEPT     { return m_perms; } 

    // modifiers
    void       type(file_type v) TINY_DB_ENGINE_NOEXCEPT       { m_value = v; }
    void       permissions(perms prms) TINY_DB_ENGINE_NOEXCEPT { m_perms = prms; }

    bool operator==(const file_status& rhs) const TINY_DB_ENGINE_NOEXCEPT
  ...
  inline bool type_present(file_status f) TINY_DB_ENGINE_NOEXCEPT
                                          { return f.type() != status_error; }
  inline bool permissions_present(file_status f) TINY_DB_ENGINE_NOEXCEPT
                                          {return f.permissions() != perms_not_known;}
  inline bool status_known(file_status f) TINY_DB_ENGINE_NOEXCEPT
                                          { return type_present(f) && permissions_present(f); }
  inline bool exists(file_status f) TINY_DB_ENGINE_NOEXCEPT
                                          { return f.type() != status_error
                                                && f.type() != file_not_found; }
  inline bool is_regular_file(file_status f) TINY_DB_ENGINE_NOEXCEPT
                                          { return f.type() == regular_file; }
  inline bool is_directory(file_status f) TINY_DB_ENGINE_NOEXCEPT
                                          { return f.type() == directory_file; }
  inline bool is_symlink(file_status f) TINY_DB_ENGINE_NOEXCEPT
                                          { return f.type() == symlink_file; }
  inline bool is_other(file_status f) TINY_DB_ENGINE_NOEXCEPT
                                          { return exists(f) && !is_regular_file(f)
                                                && !is_directory(f) && !is_symlink(f); }

# ifndef TINY_DB_ENGINE_FILESYSTEM_NO_DEPRECATED
  inline bool is_regular(file_status f) TINY_DB_ENGINE_NOEXCEPT { return f.type() == regular_file; }
# endif
  ...
  TINY_DB_ENGINE_SCOPED_ENUM_START(copy_option)
    {none=0, fail_if_exists = none, overwrite_if_exists};
  TINY_DB_ENGINE_SCOPED_ENUM_END
...
//--------------------------------------------------------------------------------------//
  ...
  namespace detail
  {
    //  We cannot pass a TINY_DB_ENGINE_SCOPED_ENUM to a compled function because it will result
    ...
    TINY_DB_ENGINE_FILESYSTEM_DECL
...
# endif
  ...
  inline
  void copy(const path& from, const path& to, system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT 
                                       {detail::copy(from, to, &ec);}
  inline
  void copy_directory(const path& from, const path& to)
                                       {detail::copy_directory(from, to);}
  inline
  void copy_directory(const path& from, const path& to, system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
                                       {detail::copy_directory(from, to, &ec);}
  inline
  void copy_file(const path& from, const path& to,   // See ticket #2925
                 TINY_DB_ENGINE_SCOPED_ENUM(copy_option) option)
  {
  ...
  inline
  void copy_file(const path& from, const path& to,   // See ticket #2925
                 TINY_DB_ENGINE_SCOPED_ENUM(copy_option) option, system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
  {
    detail::copy_file(from, to, static_cast<detail::copy_option>(option), &ec);
  }
  inline
  void copy_file(const path& from, const path& to, system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
  {
  ...
  inline
  bool create_directories(const path& p, system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
                                       {return detail::create_directories(p, &ec);}
  inline
  bool create_directory(const path& p) {return detail::create_directory(p);}

  inline
  bool create_directory(const path& p, system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
                                       {return detail::create_directory(p, &ec);}
  inline
  void create_directory_symlink(const path& to, const path& from)
                                       {detail::create_directory_symlink(to, from);}
  inline
  void create_directory_symlink(const path& to, const path& from, system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
  ...
  inline
  void create_hard_link(const path& to, const path& new_hard_link, system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
  ...
  inline
  void create_symlink(const path& to, const path& new_symlink, system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
                                       {detail::create_symlink(to, new_symlink, &ec);}
  inline
  path current_path()                  {return detail::current_path();}

  inline
  path current_path(system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT {return detail::current_path(&ec);}

  inline
  void current_path(const path& p)     {detail::current_path(p);}

  inline
  void current_path(const path& p, system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT {detail::current_path(p, &ec);}

  inline
  bool equivalent(const path& p1, const path& p2) {return detail::equivalent(p1, p2);}

  inline
  bool equivalent(const path& p1, const path& p2, system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
                                       {return detail::equivalent(p1, p2, &ec);}
  inline
  boost::uintmax_t file_size(const path& p) {return detail::file_size(p);}

  inline
  boost::uintmax_t file_size(const path& p, system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
                                       {return detail::file_size(p, &ec);}
  inline
  boost::uintmax_t hard_link_count(const path& p) {return detail::hard_link_count(p);}

  inline
  boost::uintmax_t hard_link_count(const path& p, system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
  ...
  inline
  std::time_t last_write_time(const path& p, system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
  ...
  inline
  void permissions(const path& p, perms prms, system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
  ...
  inline
  bool remove(const path& p, system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
  ...
  inline
  boost::uintmax_t remove_all(const path& p, system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
                                       {return detail::remove_all(p, &ec);}
  inline
  void rename(const path& old_p, const path& new_p) {detail::rename(old_p, new_p);}

  inline
  void rename(const path& old_p, const path& new_p, system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
  ...
  inline
  void resize_file(const path& p, uintmax_t size, system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
  ...
  inline
  space_info space(const path& p, system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
...
#if !defined(TINY_DB_ENGINE_NO_CXX11_RVALUE_REFERENCES)
  directory_entry(directory_entry&& rhs) TINY_DB_ENGINE_NOEXCEPT
  {
    m_path = std::move(rhs.m_path);
    m_status = std::move(rhs.m_status);
    m_symlink_status = std::move(rhs.m_symlink_status);
  }
  directory_entry& operator=(directory_entry&& rhs) TINY_DB_ENGINE_NOEXCEPT
  { 
    m_path = std::move(rhs.m_path);
    m_status = std::move(rhs.m_status);
    m_symlink_status = std::move(rhs.m_symlink_status);
...
# endif
  ...
  const boost::filesystem::path&  path() const TINY_DB_ENGINE_NOEXCEPT {return m_path;}
  operator const boost::filesystem::path&() const TINY_DB_ENGINE_NOEXCEPT
                                                              {return m_path;}
  file_status   status() const                                {return m_get_status();}
  file_status   status(system::error_code& ec) const TINY_DB_ENGINE_NOEXCEPT
                                                              {return m_get_status(&ec); }
  file_status   symlink_status() const                        {return m_get_symlink_status();}
  file_status   symlink_status(system::error_code& ec) const TINY_DB_ENGINE_NOEXCEPT
                                                              {return m_get_symlink_status(&ec); }

  bool operator==(const directory_entry& rhs) const TINY_DB_ENGINE_NOEXCEPT {return m_path == rhs.m_path; }
  bool operator!=(const directory_entry& rhs) const TINY_DB_ENGINE_NOEXCEPT {return m_path != rhs.m_path;} 
  bool operator< (const directory_entry& rhs) const TINY_DB_ENGINE_NOEXCEPT {return m_path < rhs.m_path;} 
  bool operator<=(const directory_entry& rhs) const TINY_DB_ENGINE_NOEXCEPT {return m_path <= rhs.m_path;} 
  bool operator> (const directory_entry& rhs) const TINY_DB_ENGINE_NOEXCEPT {return m_path > rhs.m_path;} 
...
namespace detail
{
  TINY_DB_ENGINE_FILESYSTEM_DECL
    ...
    void *& handle
#   if     defined(TINY_DB_ENGINE_POSIX_API)
    , void *& buffer
#   endif
  ); 
...
#   ifdef TINY_DB_ENGINE_POSIX_API
...
#       endif
  ...
  TINY_DB_ENGINE_FILESYSTEM_DECL void directory_iterator_construct(directory_iterator& it,
    const path& p, system::error_code* ec);
  TINY_DB_ENGINE_FILESYSTEM_DECL void directory_iterator_increment(directory_iterator& it,
...
//--------------------------------------------------------------------------------------//
  ...
  public:
    ...
    directory_iterator() TINY_DB_ENGINE_NOEXCEPT {}  // creates the "end" iterator
    ...
    directory_iterator(const path& p, system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
        : m_imp(new detail::dir_itr_imp)
          { detail::directory_iterator_construct(*this, p, &ec); }

   ~directory_iterator() {}

    directory_iterator& increment(system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
    { 
  ...
  private:
    ...
    friend TINY_DB_ENGINE_FILESYSTEM_DECL void detail::directory_iterator_construct(directory_iterator& it,
      const path& p, system::error_code* ec);
    friend TINY_DB_ENGINE_FILESYSTEM_DECL void detail::directory_iterator_increment(directory_iterator& it,
      system::error_code* ec);
    ...
    {
      TINY_DB_ENGINE_ASSERT_MSG(m_imp.get(), "attempt to dereference end iterator");
  ...
  inline
  const directory_iterator& begin(const directory_iterator& iter) TINY_DB_ENGINE_NOEXCEPT
    {return iter;}
  inline
  directory_iterator end(const directory_iterator&) TINY_DB_ENGINE_NOEXCEPT
    {return directory_iterator();}

  //  enable directory_iterator TINY_DB_ENGINE_FOREACH  -----------------------------------------//

  inline
  directory_iterator& range_begin(directory_iterator& iter) TINY_DB_ENGINE_NOEXCEPT
    {return iter;}
  inline
  directory_iterator range_begin(const directory_iterator& iter) TINY_DB_ENGINE_NOEXCEPT
    {return iter;}
  inline
  directory_iterator range_end(directory_iterator&) TINY_DB_ENGINE_NOEXCEPT
    {return directory_iterator();}
  inline
  directory_iterator range_end(const directory_iterator&) TINY_DB_ENGINE_NOEXCEPT
    {return directory_iterator();}
  }  // namespace filesystem
...
//--------------------------------------------------------------------------------------//
  ...
  TINY_DB_ENGINE_SCOPED_ENUM_START(symlink_option)
  {
    none,
  ...
  TINY_DB_ENGINE_BITMASK(TINY_DB_ENGINE_SCOPED_ENUM(symlink_option))
  ...
  namespace detail
  {
    ...
    inline
    bool recur_dir_itr_imp::push_directory(system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
    ...
    // regardless of errors.
    {
      ...
      if (ec_push_directory)
      {
        ...
        {
          TINY_DB_ENGINE_FILESYSTEM_THROW(filesystem_error(
    ...
    {
      TINY_DB_ENGINE_ASSERT_MSG(m_level > 0,
...
//--------------------------------------------------------------------------------------//
  ...
  public:
    ...
    recursive_directory_iterator() TINY_DB_ENGINE_NOEXCEPT {}  // creates the "end" iterator
    ...
    recursive_directory_iterator(const path& dir_path,
      TINY_DB_ENGINE_SCOPED_ENUM(symlink_option) opt)  // throws if !exists()
      : m_imp(new detail::recur_dir_itr_imp)
    {
      m_imp->m_options = opt;
      m_imp->m_stack.push(directory_iterator(dir_path));
    ...
    recursive_directory_iterator(const path& dir_path,
      TINY_DB_ENGINE_SCOPED_ENUM(symlink_option) opt,
      system::error_code & ec) TINY_DB_ENGINE_NOEXCEPT
    : m_imp(new detail::recur_dir_itr_imp)
    {
      m_imp->m_options = opt;
    ...
    recursive_directory_iterator& increment(system::error_code& ec) TINY_DB_ENGINE_NOEXCEPT
    {
      TINY_DB_ENGINE_ASSERT_MSG(m_imp.get(),
    ...
    int depth() const TINY_DB_ENGINE_NOEXCEPT
    { 
      TINY_DB_ENGINE_ASSERT_MSG(m_imp.get(),
    ...
    int level() const TINY_DB_ENGINE_NOEXCEPT { return depth(); }

    bool recursion_pending() const TINY_DB_ENGINE_NOEXCEPT
    {
      TINY_DB_ENGINE_ASSERT_MSG(m_imp.get(),
    ...
    bool no_push_pending() const TINY_DB_ENGINE_NOEXCEPT { return recursion_pending(); }

#   ifndef TINY_DB_ENGINE_FILESYSTEM_NO_DEPRECATED
    bool no_push_request() const TINY_DB_ENGINE_NOEXCEPT { return no_push_pending(); }
#   endif
    ...
    void disable_recursion_pending(bool value=true) TINY_DB_ENGINE_NOEXCEPT
    {
      TINY_DB_ENGINE_ASSERT_MSG(m_imp.get(),
    ...
    void no_push(bool value=true) TINY_DB_ENGINE_NOEXCEPT { disable_recursion_pending(value); }
  ...
  const recursive_directory_iterator&
    begin(const recursive_directory_iterator& iter) TINY_DB_ENGINE_NOEXCEPT
                                                  {return iter;}
  inline
  recursive_directory_iterator end(const recursive_directory_iterator&) TINY_DB_ENGINE_NOEXCEPT
                                                  {return recursive_directory_iterator();}

  //  enable recursive directory iterator TINY_DB_ENGINE_FOREACH  -------------------------------//

  inline
  recursive_directory_iterator& 
    range_begin(recursive_directory_iterator& iter) TINY_DB_ENGINE_NOEXCEPT
                                                   {return iter;}
  inline
  recursive_directory_iterator
    range_begin(const recursive_directory_iterator& iter) TINY_DB_ENGINE_NOEXCEPT
                                                   {return iter;}
  inline
  recursive_directory_iterator range_end(recursive_directory_iterator&) TINY_DB_ENGINE_NOEXCEPT
                                                  {return recursive_directory_iterator();}
  inline
  recursive_directory_iterator range_end(const recursive_directory_iterator&) TINY_DB_ENGINE_NOEXCEPT
                                                  {return recursive_directory_iterator();}
  }  // namespace filesystem
...
//  actually supports large file sizes.
  ...
  namespace detail
  {
    TINY_DB_ENGINE_FILESYSTEM_DECL bool possible_large_file_size_support();
  }
...
#endif // TINY_DB_ENGINE_FILESYSTEM3_OPERATIONS_HPP
