...
TEST_CASE("table_basic_usage","[extension]")
{
	SECTION("table_read_all")
	{
		...
		auto rt = make_shared<r_table>(eng.device, 1, 112);
		auto st = make_shared<s_table>(eng.device, 20, 224);
		...
		REQUIRE(first_r_record->a == 2);
		REQUIRE(first_r_record->b == 0);
	}
}
...
TEST_CASE("where","[query]")
{
	SECTION("linear where")
	{
		...
		auto linear_wear_rt = make_shared<r_table>(eng.device, 500, 0);
