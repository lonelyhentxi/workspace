...
namespace tinydb::filesystem {
	...
	public:
		...
		fs_result<size_t> read(valarray<byte>& buf)
		{
			assert(readable_);
			UNWRAP(inode_->read_at(offset_, buf), len, const);
			offset_ += len;
			OK(len);
		}

		fs_result<size_t> write(valarray<byte>& buf)
		{
			assert(writable_);
			UNWRAP(inode_->write_at(offset_, buf), len, const);
			offset_ += len;
			OK(len);
		}
