...
namespace {
    ...
    bool remove_directory(const path &p) // true if succeeds or not found
    {
        return BOOST_REMOVE_DIRECTORY(p.c_str())
               || not_found_error(BOOST_ERRNO);  // mitigate possible file boost::system race. See #11166
    }
...
#   endif
            ...
            void copy(const path &from, const path &to, boost::system::error_code *ec) {
                ...
                if (is_symlink(s)) {
                    ...
                    if (ec == 0)
                        BOOST_FILESYSTEM_THROW(filesystem_error("tinydb::filesystem::copy",
...
# if defined(_WIN32_WINNT) && _WIN32_WINNT < 0x0600
                error(BOOST_ERROR_NOT_SUPPORTED, new_symlink, existing_symlink, ec,
                  "tinydb::filesystem::copy_symlink");
...
#   if defined(BOOST_WINDOWS_API) && _WIN32_WINNT < 0x0600  // SDK earlier than Vista and Server 2008
                ...
                error(BOOST_ERROR_NOT_SUPPORTED, to, from, ec,
                  "tinydb::filesystem::create_directory_symlink");
#   else
...
#     if defined(BOOST_WINDOWS_API) && _WIN32_WINNT >= 0x0600
                // see if actually supported by Windows runtime dll
                if (error(!create_symbolic_link_api ? BOOST_ERROR_NOT_SUPPORTED : 0, to, from, ec,
                          "tinydb::filesystem::create_symlink"))
                    return;
#     endif
...
#         if defined(__MSL__) && (defined(macintosh) || defined(__APPLE__) || defined(__APPLE_CC__))
                      && errno != 0
#         endif
                      , ec, "tinydb::filesystem::current_path"))
                    {
                      break;
                    }
                  }
...
#   else
                ...
                error(::GetCurrentDirectoryW(sz, buf.get()) == 0 ? BOOST_ERRNO : 0, ec,
                      "tinydb::filesystem::current_path");
                return path(buf.get());
#   endif
            }
...
#   ifdef BOOST_POSIX_API
                ...
                return  s1.st_dev == s2.st_dev && s1.st_ino == s2.st_ino
                    ...
                    // taken together uniquely identify the file within the boost::system."
...
#   else  // Windows
                ...
                if (error(!::GetFileInformationByHandle(h1.handle, &info1) ? BOOST_ERRNO : 0,
                          p1, p2, ec, "tinydb::filesystem::equivalent"))
                    return false;
...
#   else  // Windows
                ...
                if (error(::GetFileAttributesExW(p.c_str(), ::GetFileExInfoStandard, &fad) == 0
                          ? BOOST_ERRNO : 0, p, ec, "tinydb::filesystem::file_size"))
                    return static_cast<boost::uintmax_t>(-1);

                if (error((fad.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) != 0
                          ? ERROR_NOT_SUPPORTED : 0, p, ec, "tinydb::filesystem::file_size"))
                    return static_cast<boost::uintmax_t>(-1);
...
#   else
                ...
                WIN32_FILE_ATTRIBUTE_DATA fad;
                if (error(::GetFileAttributesExW(p.c_str(), ::GetFileExInfoStandard, &fad) == 0
                          ? BOOST_ERRNO : 0, p, ec, "tinydb::filesystem::is_empty"))
                    return false;
...
# else  // Windows
                ...
                if (error(attr == 0 ? BOOST_ERRNO : 0, p, ec, "tinydb::filesystem::permissions"))
                    return;
                ...
                error(::SetFileAttributesW(p.c_str(), attr) == 0 ? BOOST_ERRNO : 0,
                      p, ec, "tinydb::filesystem::permissions");
# endif
            }
...
#   ifdef BOOST_POSIX_API
                ...
                for (std::size_t path_max = 64;; path_max *= 2)// loop 'til buffer large enough
                {
                  ...
                  {
                    if (ec == 0)
                      BOOST_FILESYSTEM_THROW(filesystem_error("tinydb::filesystem::read_symlink",
...
#   elif _WIN32_WINNT < 0x0600  // SDK earlier than Vista and Server 2008
                error(BOOST_ERROR_NOT_SUPPORTED, p, ec,
                      "tinydb::filesystem::read_symlink");
#   else  // Vista and Server 2008 SDK, or later
                ...
                if (error(h.handle == INVALID_HANDLE_VALUE ? BOOST_ERRNO : 0,
                          p, ec, "tinydb::filesystem::read_symlink"))
                    return symlink_path;
...
#   ifdef BOOST_POSIX_API
                ...
                if (::stat(p.c_str(), &path_stat)!= 0)
                {
                  ...
                  if (ec == 0)
                    BOOST_FILESYSTEM_THROW(filesystem_error("tinydb::filesystem::status",
                      p, error_code(errno, system_category())));
                  return fs::file_status(fs::status_error);
                }
                if (ec != 0) ec->clear();;
...
#   else  // Windows
                ...
                if (p.empty()) {
                    ...
                    if (buf.empty()
                        ...
                        error(::GetLastError(), ec, "tinydb::filesystem::temp_directory_path");
                        return path();
                    }
...
#   else
                ...
                if (error(len == 0 ? BOOST_ERRNO : 0, p, ec, "tinydb::filesystem::system_complete"))
                    return path();
                ...
                return error(get_full_path_name(p, len, big_buf.get(), &pfn) == 0 ? BOOST_ERRNO : 0,
                             p, ec, "tinydb::filesystem::system_complete")
                       ? path()
                       : path(big_buf.get());
#   endif
            }
