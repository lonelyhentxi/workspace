...
fn main() {
    ...
    let lower_right = parse_complex(&args[4]).expect("error lower right corner point");
...
fn write_image(filename: &str, pixels: &[u8], bounds: (usize, usize)) -> Result<(), Box<dyn Error>> {
