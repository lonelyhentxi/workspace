...
template <size_t Dim, typename T, typename U> vec<Dim, T>
	...
	template <size_t Dim, typename T, typename U> vec<Dim, T>
		...
		public:
			...
			void set_row(size_t idx, vec<DimCols> v) {
				rows[idx] = std::move(v);
			}
			...
			T cofactor(size_t row, size_t col) const {
				return get_minor(row, col).get_minor((row + col) % 2 ? -1 : 1);
			}
		...
		template<size_t R1, size_t C1, size_t C2, typename T> mat<R1, C2, T>
			...
			template<size_t DimRows, size_t DimCols, typename T>
			mat<DimRows, DimCols, T> operator/(const mat<DimRows> &lhs, const T&rhs) {
				mat<DimRows, DimCols, T> rets;
				for (size_t i = DimRows; i--;) {
					rets.set_row(rets.row(i));
				}
				return rets;
			}

			template<size_t DimRows, size_t DimCols, typename T> std::ostream& << (std::ostream &out, mat<DimRows, DimCols, T>& m) {
				for (size_t i = 0; i < DimRows; i++) {
