...
pub type ParsingResult = Result<(Vec<ASTNode>, Vec<TokenType>), String>;
...
fn parse_function(
    ...
    match prototype.ftype {
        BinaryOp(ref symbol, precedence) => {
            settings
                .operator_precedence
                .insert(symbol.clone(), precedence);
        }
        _ => (),
    };
...
fn parse_prototype(
    ...
    Good(Prototype { name, args, ftype }, parsed_tokens)
}
...
fn parse_conditional_expr(
    ...
    expect_token!([Then, Then, ()] <= tokens, parsed_tokens, "expected then");
    let then_expr = parse_try!(parse_expr, tokens, settings, parsed_tokens);

    expect_token!([Else, Else, ()] <= tokens, parsed_tokens, "expected else");
