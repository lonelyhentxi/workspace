...
def read_examples(input_file, is_training):
    ...
    examples=[]
    for val in df[['id','comment','label']].values:
...
def main():
    ...
    if args.do_test:
        ...
        for file,flag in [('dev.csv','dev'),('test.csv','test')]:
            ...
            if flag=='test':
                df=pd.read_csv(os.path.join(args.data_dir, file), sep="\t")
                df['label_0']=logits[:,0]
                df['label_1']=logits[:,1]
                df[['id','label_0','label_1']].to_csv(os.path.join(args.output_dir, "sub.csv"),index=False, sep='\t')
